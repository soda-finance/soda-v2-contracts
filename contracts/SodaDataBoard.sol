...
import "./strategies/CreateMoreSoda.sol";
...
contract SodaDataBoard is Ownable {
    ...
    function getAPY(uint256 _poolId, address _token, bool _isLPToken) public view returns(uint256) {
        ...
        uint256 MK_STRATEGY_CREATE_MORE_SODA = 0;
        CreateMoreSoda createMoreSoda = CreateMoreSoda(sodaMaster.strategyByKey(MK_STRATEGY_CREATE_MORE_SODA));
        (uint256 allocPoint,) = createMoreSoda.poolMap(address(vault));
        uint256 totalAlloc = createMoreSoda.totalAllocPoint();
        ...
        uint256 factor = 20;  // 0.02 SODA per block

        if (vaultSupply == 0) {
            // Assume $1 is put in.
            return getSodaPrice() * factor * 5760 * 100 * allocPoint / totalAlloc / 1e6 / 1e3;
        }
        ...
        if (_isLPToken) {
            ...
            return getSodaPrice() * factor * 2250000 * 100 * allocPoint * 1e18 / totalAlloc / lpPrice / vaultSupply / 1e3;
        } else {
            ...
            return getSodaPrice() * factor * 2250000 * 100 * allocPoint * 1e18 / totalAlloc / tokenPrice / vaultSupply / 1e3;
        }
    }
