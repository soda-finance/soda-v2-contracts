...
interface ISodaToken is IERC20 {
...
contract CreateMoreSoda is IStrategy, Ownable {
    using SafeMath for uint256;

    uint256 public constant ALL_BLOCKS_AMOUNT = 5000000;
    uint256 public constant SODA_PER_BLOCK = 2 * 1e16;  // 0.02 per block
    ...
    function _update(address _vault) public {
        ...
        ISodaToken(sodaMaster.soda()).mint(sodaMaster.dev(), allReward.div(10));  // 10% goes to dev.
        uint256 farmerReward = allReward.sub(allReward.div(10));

        ISodaToken(sodaMaster.soda()).mint(address(this), farmerReward);  // 90% goes to farmers.
