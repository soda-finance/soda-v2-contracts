...
contract GovernableSodaVault is SodaVault {

    constructor (SodaMaster _sodaMaster, string memory _name, string memory _symbol) SodaVault(_sodaMaster, _name, _symbol) public  {
    }

    /// @notice Creates `_amount` token to `_to`. Must only be called by SodaPool.
    function mintByPool(address _to, uint256 _amount) public override {
        super.mintByPool(_to, _amount);
